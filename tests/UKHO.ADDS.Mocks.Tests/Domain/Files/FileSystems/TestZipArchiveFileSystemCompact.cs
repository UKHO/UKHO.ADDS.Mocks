...
namespace UKHO.ADDS.Mocks.Tests.Domain.Files.FileSystems
{
    public class TestZipArchiveFileSystemCompact
    {
        ...
        public TestZipArchiveFileSystemCompact() => fs = new ZipArchiveFileSystem();
        ...
        public void TestFileExceptions()
        {
            ...
            Assert.Throws<FileNotFoundException>(() => fs.ReplaceFile("/1.txt", "/1.txt", default, true));
        ...
        public void TestOpenFileMultipleRead()
        {
            ...
            Assert.Equal('c', (char)stream1.ReadByte());

            Assert.Equal('c', (char)stream2.ReadByte());
            ...
            writeSystem = new ZipArchiveFileSystem(memStream);
        ...
        public void TestEnumeratePaths()
        {
            ...
            folders.Sort();
            Assert.Equal(new List<UPath> { "/dir1/a" }
                , folders);
            ...
            Assert.Equal(new List<UPath> { "/dir1/a/file10.txt", "/dir1/a1/file11.txt" }
                , files);

            files = fs.EnumeratePaths("/", "file?0.txt", SearchOption.AllDirectories, SearchTarget.File).ToList();
            files.Sort();

            Assert.Equal(new List<UPath> { "/dir1/a/file10.txt", "/dir2/file20.txt" }
                , files);
        }
        ...
        [Fact]
        public void TestInvalidCharacter() => Assert.Throws<NotSupportedException>(() => fs.CreateDirectory("/toto/ta:ta"));
#if !NET472
        [Fact]
