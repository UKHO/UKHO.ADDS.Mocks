...
namespace UKHO.ADDS.Mocks.Domain.Files.FileSystems
{
    ...
    ///     This mount filesystem supports also an optionnal fallback delegate FileSystem if a path was not found through a
    ///     mount
    /// </summary>
    ...
    public class MountFileSystem : ComposeFileSystem
    {
        ...
        /// <param name="owned">
        ///     True if <paramref name="defaultBackupFileSystem" /> and mounted filesystems should be disposed when
        ...
        ///     The path inside of <paramref name="fileSystem" /> that refers to the file at
        ///     <paramref name="path" />.
        /// </param>
        ...
        /// <inheritdoc />
        protected override DateTime GetCreationTimeImpl(UPath path) => TryGetMountOrNext(ref path)?.GetCreationTime(path) ?? DefaultFileTime;
        ...
        /// <inheritdoc />
        protected override DateTime GetLastAccessTimeImpl(UPath path) => TryGetMountOrNext(ref path)?.GetLastAccessTime(path) ?? DefaultFileTime;
        ...
        /// <inheritdoc />
        protected override DateTime GetLastWriteTimeImpl(UPath path) => TryGetMountOrNext(ref path)?.GetLastWriteTime(path) ?? DefaultFileTime;
        ...
        private IFileSystem? TryGetMountOrNext(ref UPath path) => TryGetMountOrNext(ref path, out var _);
        ...
        private sealed class DebuggerProxy
        {
            ...
            public DebuggerProxy(MountFileSystem fs) => _fs = fs;

            [DebuggerBrowsable(DebuggerBrowsableState.RootHidden)] public KeyValuePair<string, IFileSystem>[] Mounts => _fs._mounts.Select(x => new KeyValuePair<string, IFileSystem>(x.Key.ToString(), x.Value)).ToArray();
