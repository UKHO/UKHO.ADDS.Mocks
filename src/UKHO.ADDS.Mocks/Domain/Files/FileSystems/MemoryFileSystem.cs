...
{
    /// <summary>
    ///     Provides an in-memory <see cref="IFileSystem" /> compatible with the way a real <see cref="PhysicalFileSystem" />
    ///     is working.
    /// </summary>
    ...
    public class MemoryFileSystem : FileSystem
    {
        ...
        protected virtual MemoryFileSystem CloneImpl() => new(this);

        protected override string DebuggerDisplay() => $"{base.DebuggerDisplay()} {_rootDirectory.DebuggerDisplay()}";
        ...
        /// <inheritdoc />
        protected override void MoveDirectoryImpl(UPath srcPath, UPath destPath) => MoveFileOrDirectory(srcPath, destPath, true);
        ...
        protected override void ReplaceFileImpl(UPath srcPath, UPath destPath, UPath destBackupPath, bool ignoreMetadataErrors)
        {
            ...
            var paths = new List<KeyValuePair<UPath, int>> { new(srcPath, 0), new(destPath, 1) };
        ...
        /// <inheritdoc />
        protected override void MoveFileImpl(UPath srcPath, UPath destPath) => MoveFileOrDirectory(srcPath, destPath, false);
        ...
        /// <inheritdoc />
        protected override DateTime GetCreationTimeImpl(UPath path) => TryFindNodeSafe(path)?.CreationTime ?? DefaultFileTime;
        ...
        /// <inheritdoc />
        protected override DateTime GetLastAccessTimeImpl(UPath path) => TryFindNodeSafe(path)?.LastAccessTime ?? DefaultFileTime;
        ...
        /// <inheritdoc />
        protected override DateTime GetLastWriteTimeImpl(UPath path) => TryFindNodeSafe(path)?.LastWriteTime ?? DefaultFileTime;
        ...
        /// <inheritdoc />
        protected override void CreateSymbolicLinkImpl(UPath path, UPath pathToTarget) => throw new NotSupportedException("Symbolic links are not supported by MemoryFileSystem");
        ...
        private void CreateDirectoryNode(UPath path) => ExitFindNode(EnterFindNode(path, FindNodeFlags.CreatePathIfNotExist | FindNodeFlags.NodeShared));
        ...
        private NodeResult EnterFindNode(UPath path, FindNodeFlags flags, params NodeResult[] existingNodes) => EnterFindNode(path, flags, null, existingNodes);
        ...
        private void EnterFileSystemShared() => _globalLock.EnterShared(UPath.Root);

        private void ExitFileSystemShared() => _globalLock.ExitShared();

        private void EnterFileSystemExclusive() => _globalLock.EnterExclusive();

        private void ExitFileSystemExclusive() => _globalLock.ExitExclusive();

        private void EnterSharedDirectoryOrBlock(DirectoryNode node, UPath context) => EnterShared(node, context, true, FileShare.Read);
        ...
        private void EnterExclusiveDirectoryOrBlock(DirectoryNode node, UPath context) => EnterExclusive(node, context, true);

        private void EnterExclusive(FileSystemNode node, UPath context) => EnterExclusive(node, context, node is DirectoryNode);

        private void EnterShared(FileSystemNode node, UPath context, FileShare share) => EnterShared(node, context, node is DirectoryNode, share);
        ...
        private void ExitShared(FileSystemNode node) => node.ExitShared();
        ...
        private void ExitExclusive(FileSystemNode node) => node.ExitExclusive();
        ...
        private class ListFileSystemNodes : List<KeyValuePair<string, FileSystemNode>>, IDisposable
        {
            ...
            public ListFileSystemNodes(MemoryFileSystem fs) => _fs = fs ?? throw new ArgumentNullException(nameof(fs));
        ...
        private class DirectoryNode : FileSystemNode
        {
            ...
            public override string DebuggerDisplay() => Name is null ? $"Count = {_children.Count}{base.DebuggerDisplay()}" : $"Folder: {Name}, Count = {_children.Count}{base.DebuggerDisplay()}";

            private sealed class DebuggerProxyInternal
            {
                private readonly DirectoryNode _directoryNode;

                public DebuggerProxyInternal(DirectoryNode directoryNode) => _directoryNode = directoryNode;

                [DebuggerBrowsable(DebuggerBrowsableState.RootHidden)] public FileSystemNode[] Items => _directoryNode._children.Values.ToArray();
            }
        }
        ...
        private sealed class FileNode : FileSystemNode
        {
            ...
            public override string DebuggerDisplay() => $"File: {Name}, {Content.DebuggerDisplay()}{base.DebuggerDisplay()}";
        ...
        private sealed class MemoryFileStream : Stream
        {
            ...
            ~MemoryFileStream() => Dispose(false);
            ...
            public override void Flush() => CheckNotDisposed();
        ...
        private class FileSystemNodeReadWriteLock
        {
            ...
            public void EnterShared(UPath context) => EnterShared(FileShare.Read, context);
            ...
            public virtual string DebuggerDisplay() => _sharedCount < 0 ? " (exclusive lock)" : _sharedCount > 0 ? $" (shared lock: {_sharedCount})" : string.Empty;
        }
        ...
        private sealed class DebuggerProxy
        {
            ...
            public DebuggerProxy(MemoryFileSystem fs) => _fs = fs;

            [DebuggerBrowsable(DebuggerBrowsableState.RootHidden)] public FileSystemNode[] Items => _fs._rootDirectory._children.Select(x => x.Value).ToArray();
        }
    }
}
